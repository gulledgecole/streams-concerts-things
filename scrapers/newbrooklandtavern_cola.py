...
def scrape_newbrooklandtav(url):
    ...
    if data.status_code == 200:
        ...
        for div_element in div_elements:
            ...
            bands = [bands.split('*')[0].strip().split('-')[0].strip()] # for some reason, this website adds a bunch of stuff after the bandname with * and -.
            bands = [item.strip() for item in bands[0].split(' w/')]
            bands = [item.strip() for item in bands[0].split(',')] # this website seperates bands by "," in title.
